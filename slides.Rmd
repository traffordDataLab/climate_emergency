...
co2_emissions_sector <- co2_emissions %>%
  filter(area_code == params$la, period == 2024, sector != "LULUCF") %>%
...
ggplot(data = vertices) + 
  ...
  geom_text(data = layout, aes(x, y), label = paste0(str_wrap(co2_emissions_sector$sector, width = 15), "\n", co2_emissions_sector$percent, "%"), 
            check_overlap = TRUE, color = "#000000", size = 5, face = "bold") + 
  ...
  labs(title = "Carbon dioxide emissions by main sector",
       subtitle = paste0(id, ", 2024"),
       caption = "Excluding LULUCF\nSource: BEIS, DESNZ") +
  coord_equal() +
  theme_void() +
  theme(plot.title = element_text(size = 24, face = "bold", hjust = 0),
      plot.subtitle = element_text(size = 20, hjust = 0, margin = margin(9, 0, 9, 0)),
      plot.caption = element_text(size = 15, color = "grey50", hjust = 1, margin = margin(t = 15)))
```
...
ggplot(industrial_commercial_energy_consumption, aes(x = value, y = group)) +
  ...
  geom_text(aes(label = comma(value, accuracy = 1)), fontface = "bold", colour = "white", size = 5) +
...
ggplot(domestic_property_build_period, aes(x = factor(period), y = value)) +
  ...
  labs(x = NULL, 
       ...
       subtitle = paste0(id, ", up to March 2025"),
...
cat(paste0(percent(pull(summarise(filter(mutate(domestic_epcs_energy_efficiency, percent = value/sum(value)), efficiency %in% c("A","B","C")), abc = sum(percent))), accuracy = 0.1), " of registered domestic Energy Performance Certificates in ", id, " have a rating of A, B or C, over a 10 year period to 2026/Q1."))
```
...
fuel_poverty_test = ifelse(grepl("S", params$la, fixed = TRUE) | grepl("W", params$la, fixed = TRUE) | grepl("N", params$la, fixed = TRUE),FALSE,TRUE)
```

```{r eval=fuel_poverty_test, dpi=300, fig.cap = "In 2024, 9.9% of all households in England (2.47 million households) were in fuel poverty."}
ggplot(fuel_poverty) + 
...
ggplot(data = background_air_pollution, 
       aes(fill = no22024, color = no22024)) +
  geom_sf(data = la, fill = NA, color = "#212121", size = 0.5) +
...
ggplot(data = background_air_pollution, 
       aes(fill = pm102024g, color = pm102024g)) +
  geom_sf(data = la, fill = NA, color = "#212121", size = 0.5) +
  geom_sf(color = NA, alpha = 0.8) +
  scale_fill_viridis_c(option = "magma", direction = -1,
                       guide = guide_colorbar(
...
ggplot(data = background_air_pollution, 
       aes(fill = pm252024g, color = pm252024g)) +
  geom_sf(data = la, fill = NA, color = "#212121", size = 0.5) +
  geom_sf(color = NA, alpha = 0.8) +
  scale_fill_viridis_c(option = "magma", direction = -1,
                       guide = guide_colorbar(
...
ggplot(onshore_wind, aes(lon, lat)) + 
  ...
  labs(x = NULL, y = NULL,
       ...
       subtitle = paste0(distinct(onshore_wind, area_name), " 2025/26"),
...
cat(paste0("There have been ", pull(non_domestic_rhi, value), " non-domestic Renewable Heat Incentive applications accredited in ", id, " between November 2011 and March 2024. This is the final number as the program is closed."))
```
...
cat(paste0("A total of ", pull(domestic_rhi, value), " applications have been accredited for payment in ", id, " between April 2014 and March 2024. This is the final number as the program is closed."))
```

#### Boiler Upgrade Scheme
```{r}
boiler_upgrade_scheme <- boiler_upgrade_scheme %>% 
  filter(area_code == params$la) 

boiler_upgrade_scheme_test = nrow(boiler_upgrade_scheme) != 0
```

```{r eval=boiler_upgrade_scheme_test, dpi=300, fig.cap= paste(sum(boiler_upgrade_scheme$value, na.rm = TRUE), ' Boiler Upgrade Scheme (BUS) redemptions paid so far in ', filter(lookup, area_code == params$la)$area_name)}
ggplot(boiler_upgrade_scheme, aes(x = factor(period), y = value)) +
  ...
  labs(x = NULL, 
       ...
       subtitle = paste0(id, ", 2022/23 - 2025/26"),
...
```{r eval=ulev_test, dpi=300, fig.cap= paste(round(max(ulev$percent)*100,1), '% of all licensed vehicles in ', filter(lookup, area_code == params$la)$area_name, 'are Ultra Low Emissions Vehicles (ULEV) in 2025')}
ggplot(ulev %>% filter(period >=2015), aes(x = period, y = percent)) +
  ...
  labs(x = NULL, y = NULL, 
       ...
       subtitle = paste0(id, ", 2015 - ", max(ulev$period)),
...
```{r eval=ev_test, dpi=300, fig.cap= paste(round(max(ev$percent)*100,1), '% of all licensed vehicles in ', filter(lookup, area_code == params$la)$area_name, 'are Licensed electric cars and vans in 2025')}
ggplot(ev %>% filter(period >=2015), aes(x = period, y = percent)) +
  ...
  labs(x = NULL, y = NULL, 
       ...
       subtitle = paste0(id, ", 2015 - ", max(ev$period)),
       caption = "Source: DfT and DVLA") +
  theme_x() +
  theme(axis.title.y = element_text(hjust = 0))
```

#### Residents who cycle, 2023
```{r}
cycling <- cycling %>% 
  filter(area_code == params$la) 
...
cycling %>% 
  ...
  kable(caption = "11.9% of adults in England cycle at least once a week. Source: Department for Transport")
```

#### Residents who walk, 2023
```{r}
walking <- walking %>% 
  filter(area_code == params$la) 
...
walking %>% 
  ...
  mutate(
         value = round(value,1)) %>% 
    select(`How often?` = frequency, `%` = value) %>% 
  kable(caption = "68% of adults in England walk at least once a week. Source: Department for Transport")
```
...
cat(paste0("The extent of the designated Green Belt in ", id, " at 31 March 2025 was estimated at ", comma(pull(green_belt %>% filter(indicator == "Green Belt area"), value)), " hectares, or approximately ", round(pull(green_belt %>% filter(indicator == "Green Belt area"), value)/pull(green_belt %>% filter(indicator == "Total area"), value)*100, 1), "% of the land area of ", id, "."))
```

#### Further resources

- [Carbon and Co-benefits Decision Support Tool](https://cast.ac.uk/cast-tools/carbon-and-co-benefits-decision-support-tool/)
- [9 things you can do about climate change](https://www.imperial.ac.uk/stories/climate-action/)
- [Friends of the Earth Local action](https://groups.friendsoftheearth.uk)