...
```{r eval=boiler_upgrade_scheme_test, dpi=300, fig.cap= paste(sum(boiler_upgrade_scheme$value), ' Boiler Upgrade Scheme (BUS) redemptions paid so far in ', filter(lookup, area_code == params$la)$area_name)}
...
```{r eval=ev_test, dpi=300, fig.cap= paste(round(max(ev$percent)*100,1), '% of all licensed vehicles in ', filter(lookup, area_code == params$la)$area_name, 'are Ultra Low Emissions Vehicles (ULEV) in 2025')}
