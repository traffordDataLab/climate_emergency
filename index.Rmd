...
library(tidyverse) ; library(scales) ; library(ggsci) ; library(sf) ; library(ggrepel) ; library(lubridate) ; library(rvest) ; library(packcircles) ; library(treemapify) ; library(ggiraph) ; library(viridis) ; library(leaflet) ; library(leaflet.extras) ; library(htmlwidgets) ; library(htmltools)
...
large_point_sources <- read_csv("data/large_point_sources.csv") %>% 
  ...
  mutate(popup = str_c("<strong>Site: </strong>", site, "<br/>",
                       "<strong>Operator: </strong>", operator, "<br/>",
                       "<strong>Sector: </strong>", sector, "<br/>",
                       "<strong>CO<sub>2</sub> emissions (2017): </strong>", comma(value), " tonnes") %>%
           map(HTML))

leaflet(large_point_sources) %>% 
  setView(lng = -2.330256, lat = 53.421829, zoom = 11) %>% 
  addProviderTiles(providers$CartoDB.Positron) %>% 
  addPolygons(data = la, fillOpacity = 0, color = "#212121", weight = 2, opacity = 1) %>% 
  addCircles(lng = ~lon, lat = ~lat, radius = ~sqrt(value), stroke = TRUE, weight = 1, color = "#FFFFFF", fillColor = "#212121", fillOpacity = 0.7, label = ~popup) %>%
